...
on walking. I will take the median of each group and compare them which
will allow the data to not be impacted by outliers, while also taking
the minimum and maximum, so the range will also be helpful, but I do not
need to add numbers together.
...
group_by(on_the_phone) |> # group by yes or no on the phone
  summarize(
    minimum = min(time_it_takes_me_to_walk_to_tunnel_min, na.rm = TRUE),
    q1 = quantile(time_it_takes_me_to_walk_to_tunnel_min, 0.25, na.rm = TRUE),
    median = median(time_it_takes_me_to_walk_to_tunnel_min, na.rm = TRUE),
    q3 = quantile(time_it_takes_me_to_walk_to_tunnel_min, 0.75, na.rm = TRUE),
    maximum = max(time_it_takes_me_to_walk_to_tunnel_min, na.rm = TRUE)
  )
...
if I was not on the phone (red: no, green: yes). **does this caption
look correct**
...
ft <- flextable(head(data_summary_rounded),
                col_keys = c("on_the_phone", "minimum", "q1", "median", "3", "maximum")) |> 
  set_header_labels(mean = "Median", on_the_phone = "On the phone", q1 = "First quartile", q3 = "Third quartile", minimum = "Minimum", maximum = "Maximum") |> 
  autofit()

 ft <- set_caption(ft, caption = "Table 1: Walking Data Summary")

ft
```

**should I take out variables I don't need from the table and from my
code?**
